...
namespace IFAD_v1._1
{
    public partial class StockTransfer : Form
    {
        ...
        private void treeViewPurchaseItemSales_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                while (reader12.Read())
                {
                   ...
                   // textBoxProCate.Text = reader12["Code"].ToString();
                }
                reader12.Close();
                connection12.Close();
        ...
        private double Currently_Stock()
        {
            ...
            string queryPross = "SELECT * FROM Product WHERE ID = '" + Convert.ToInt32(textBoxPdoductID.Text) + "'";
        ...
        public void TempSales(string temp_product_code,string name, double qantity, double temp_sale_price, double temp_purchase_price, string invoice_no, double discount, double vat, double temp_sales_with_vat, double price_total, int temp_pro_id)
        {
            ...
            SqlConnection connection = new SqlConnection(conStr);
            string query = "INSERT INTO TempSales(TempSalesCompanyID,TempSalesNo,TempSalesDate,TempSaleTime,TempSalesCustomerID,TempSalesRemarks,TempSalesProductID,TempSalesPurchasePrice,TempSalesSalePrice,TempSalesQuantity,TempSalesProductDiscount,TempSalesTotal,TempSalesCustomerName,TempSalesSoldBy, TempSalesVatRate, TempSalesVatTotal, TempSalesPuechaseBy, TempSalesPurchaseByContact) VALUES('" + CompanyID+"','" + invoice_no + "','" + date + "','" + textBoxTime.Text + "','" + textBoxRemarks.Text + "','" + textBoxPdoductID.Text + "','" + temp_purchase_price + "','" + temp_sale_price + "','" + qantity + "','" + discount + "','" + price_total + "','"+ currentuser + "', '"+ vat +"', '" + temp_sales_with_vat + "', '" + textBoxTransferTo.Text + "', '" + textBoxReceivedBy.Text + "')";
            SqlCommand command = new SqlCommand(query, connection);
            connection.Open();
        ...
        {
            SqlConnection connection1 = new SqlConnection(conStr);
            string query1 = "UPDATE Product SET Stock = ((SELECT Stock FROM Product WHERE ID = '" + pro_id + "' ) - '"+quantity+"') WHERE ID = '" + pro_id + "'";
        ...
        {
            textBoxItemTotal.Text = (dataGridViewSales.Rows.Count).ToString();

            int sum = 0;
            for (int i = 0; i < dataGridViewSales.Rows.Count; ++i)
            {
                sum += Convert.ToInt32(dataGridViewSales.Rows[i].Cells[3].Value);
            }
            textBoxProductTotal.Text = sum.ToString();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxTransferTo.Text == "")
            {
                MessageBox.Show("Please Give Requester Name");
            }

            else if (textBoxReference.Text == "")
            {
                MessageBox.Show("Please Give Reference Information");
            }
            ...
            {
                if (dataGridViewSales.Rows.Count > 0 && isEdited == 0)
                {
                    foreach (DataGridViewRow row in dataGridViewSales.Rows)
                    {
                        ...
                        {
                            rowIndex = -1;
                            if (row.Cells["ID"].Value.ToString().Equals(textBoxPdoductID.Text))
                            {
                                rowIndex = row.Index;
                                DataGridViewRow newDataRow = dataGridViewSales.Rows[rowIndex];
                                
                                double sum = Convert.ToDouble(newDataRow.Cells[3].Value) + Convert.ToDouble(textBoxQuantity.Text);
                                if (sum > Convert.ToDouble(textBoxCurrentStock.Text))
                                {
                                    //MessageBox.Show("Insufficient Quantity");

                                    DialogResult dr = MessageBox.Show("Insufficient Quantity", "Insufficient Quantity", MessageBoxButtons.OK, MessageBoxIcon.Question);
                ...
                if (isEdited != 1)
                {
                    if (Convert.ToDouble(textBoxQuantity.Text) > Convert.ToDouble(textBoxCurrentStock.Text))
                    {
                        MessageBox.Show("Insufficient Quantity");
                    ...
                    {
                    int n = dataGridViewSales.Rows.Add();
                    dataGridViewSales.Rows[n].Cells[0].Value = textBoxPdoductID.Text;
                    dataGridViewSales.Rows[n].Cells[1].Value = textBoxProCate.Text;
                    dataGridViewSales.Rows[n].Cells[2].Value = textBoxProductName.Text;
                    dataGridViewSales.Rows[n].Cells[3].Value = textBoxQuantity.Text;
                    dataGridViewSales.Rows[n].Cells[4].Value = textBoxUnitType.Text;

                    SeeTotal_Component();

                    textBoxProductName.Text = textBoxPdoductID.Text = textBoxPrice.Text = textBoxUnitType.Text = textBoxProCate.Text = "";
       ...
       int PaymentType = 0;
        ...
        //private void InsertTempAmount(int Companyid, string  CurrentUserSales, string Remarks)
        //{
        //    TempAmountTruncate();
        //    SqlConnection con1 = new SqlConnection(conStr);
        //    con1.Open();
        //    string sql1 = @"INSERT INTO TempSalesAmount(CompanyID, CustomerID, TotalVat, NetPayable, CashPaid, ReturnAmount, DueAmount,CurrentUserSales,Remarks) VALUES('" + Companyid + "','" + CurrentUserSales + "','" + Remarks + "')";
        ...
        //    string sql1 = @"UPDATE TempSales SET TempSalesRemarks = '"+ textBoxRemarks .Text + "' WHERE TempSalesCompanyID = '"+CompanyID+"'";      // Change
        //    SqlCommand cmd1 = new SqlCommand(sql1, con1);
        //    cmd1.ExecuteNonQuery();
        //    con1.Close();
        //}
       // public string SerialNo = "";
        ...
        //    string sql1 = @"UPDATE TempSales SET TempSalesReference='" + textBoxReference.Text + "',TempSalesNo='" + SerialNo + "', TempSalesPaymentType = '" + PaymentType +"' WHERE TempSalesCompanyID = '" + CompanyID + "'";    // Change
        //    SqlCommand cmd1 = new SqlCommand(sql1, con1);
        //    cmd1.ExecuteNonQuery();
        //    con1.Close();
        //}
        ...
        private int CountTransferNo()
        {
            int no = 0;
            SqlConnection con = new SqlConnection(conStr);
            string query = "SELECT COUNT(DISTINCT(TransferNo)) as 'No' FROM StockTransfer";
            SqlCommand command = new SqlCommand(query, con);
            con.Open();
            SqlDataReader reader = command.ExecuteReader();
        ...
        {
            int countsales = CountTransferNo();
            ...
            else
            {
                ...
                foreach (DataGridViewRow row in dataGridViewSales.Rows)
                {
                    ...
                    double quantity = Convert.ToDouble(dataGridViewSales.Rows[n].Cells[3].Value);

                    string query = "INSERT INTO StockTransfer (CompanyID, TransferNo, TransferDate, TransferTime, TransferTo, TransferRemarks, TransferReference, TransferProductID, TransferQuantity, TransferBy, TransferReceivedBy) VALUES ('" + CompanyID + "', '" + SerialNo + "', '" + textBoxDate.Text + "', '" + textBoxTime.Text + "', '"+ textBoxTransferTo.Text +"' , '" + textBoxRemarks.Text + "', '" + textBoxReference.Text + "', '" + prod_id + "', '" + quantity + "', '" + currentuser + "','" + textBoxReceivedBy.Text+ "')";

                    SqlCommand cmd = new SqlCommand(query, con);

                    cmd.ExecuteNonQuery();
                   
                    UpdateProductDetails(prod_id, quantity );
                    n++;
                ...
                textBoxTransferTo.Text = textBoxReceivedBy.Text = textBoxRemarks.Text = textBoxReference.Text = textBoxItemTotal.Text = textBoxProductTotal.Text = "";
                clearAll();
              ...
              //  InsertTempAmount(Companyid, CurrentUserSales,Remarks);
              ...
              //   SerialNo = "";
              //   Temp_Sales_Truncate();
              //   textBoxRemarks.Text = textBoxRequestBy.Text = textBoxPruchaseByContact.Text = textBoxReference.Text = textBoxPdoductID.Text = textBoxProCate.Text = string.Empty;
        ...
        private void dataGridViewSales_DoubleClick_1(object sender, EventArgs e)
        {
            ...
            string conStr11 = ConfigurationManager.ConnectionStrings["PosConString"].ToString();
            SqlConnection connection1211 = new SqlConnection(conStr11);
            string query1211 = "SELECT * FROM Product WHERE Product.ID = '"+textBoxPdoductID.Text+ "'";
            SqlCommand command11211 = new SqlCommand(query1211, connection1211);
            ...
            string query13 = "SELECT CategorySub.SubCategoryName FROM CategorySub INNER JOIN Product ON CategorySub.SubCategoryID = Product.SubCategoryID WHERE Product.ID = '" + textBoxPdoductID.Text + "'";
            SqlCommand command113 = new SqlCommand(query13, connection1211);
        ...
        {
            textBoxQuantity.Text = textBoxProductName.Text = textBoxProCate.Text = textBoxPrice.Text = textBoxPdoductID.Text = textBoxUnitType.Text = textBoxCurrentStock.Text = textBoxProductSearch.Text = "";
        }
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridViewSales.Rows.Count > 0)
                {
                    ...
                    newDataRow.Cells[1].Value = textBoxProCate.Text;
                    newDataRow.Cells[2].Value =textBoxProductName.Text;
                    newDataRow.Cells[3].Value =textBoxQuantity.Text;
                    newDataRow.Cells[4].Value =textBoxUnitType.Text;
                    clearAll();
                }
                SeeTotal_Component();
        ...
        //    {
        //        this.ActiveControl = buttonTransfer;
        //    }
        //}
