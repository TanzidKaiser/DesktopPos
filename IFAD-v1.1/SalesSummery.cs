...
{
    public partial class SalesSummery : Form
    {
        public SalesSummery()
        {
            InitializeComponent();
        ...
        private void SalesSummery_Load_1(object sender, EventArgs e)
        {
        ...
        private void DataGrid(string date1, string date2)
        {
            ...
            cmd.CommandText = "SELECT Sales.SalesDate as 'Date', Sales.SalesCustomerName as 'Customer', Sales.SalesNo as 'Code',  Product.Name as 'Name', Product.ID as 'Product_ID', Sales.SalesSalePrice as 'Price', Sales.SalesQuantity as 'Quantity', Sales.SalesProductDiscount as 'Discount', Sales.SalesRemarks as 'Remarks', Sales.SalesTotal as 'Total', (case when Sales.SalesVatTotal-Sales.SalesReceivedAmount>0 then Sales.SalesVatTotal-Sales.SalesReceivedAmount else 0 END) as 'Due_Amount' FROM Sales, Product WHERE Product.ID = Sales.SalesProductID AND Sales.SalesDate BETWEEN '" + date1 + "' AND '" + date2 + "' ORDER BY Sales.SalesDate";
            cmd.ExecuteNonQuery();
        ...
        string reportPath = ReportPath.rPath;
        private void CrystalReportCall()
        {
            ...
            string rPath = reportPath + "CrystalReportSalesSummeryA4.rpt";
            cryRpt.Load(rPath);
            ...
            TextboxValue("@start_date", date1, myParameterField1, myDiscreteValue1, myParameterFields);
            TextboxValue("@end_date", date2, myParameterField2, myDiscreteValue2, myParameterFields);
        ...
        private void StorePrecedureCall()
        {
            ...
            con.Open();
            string query = "EXEC sp_SalesSummeryDateWise @start_date = '" + date1 + "', @end_date = '"+ date2 + "'";
        ...
        private void Temp_Sales_Stock()
        {
            foreach (DataGridViewRow row in dataGridViewSalesReport.Rows)
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO TempSalesStock VALUES('1',@Date,@Code, @Name,@Product_ID,@Price,@Quantity,@Discount,@Due_Amount,@Total,@Customer,@Remarks,'" + dateTimePicker1.Text + "','" + dateTimePicker2.Text + "')", con))
                    {
        ...
        {
            con.Open();
            string sql = @"TRUNCATE TABLE TempSalesStock";
        ...
        private void PrintReport()
        {
            ...
            TableLogOnInfo crtableLogoninfo = new TableLogOnInfo();
            ConnectionInfo crConnectionInfo = new ConnectionInfo();
            string rPath = ReportPaths + "CrystalReportSalesSummeryA4.rpt";
            cryRpt.Load(rPath);
            //cryRpt.Load(rPath);
            crConnectionInfo.ServerName = ConfigurationManager.ConnectionStrings["cryServer"].ToString();
            crConnectionInfo.DatabaseName = ConfigurationManager.ConnectionStrings["cryDatabase"].ToString();
        ...
        {
            con.Open();
            string sql = @"TRUNCATE TABLE TempInvoiceSalesReport";
            SqlCommand cmd = new SqlCommand(sql, con);
            cmd.ExecuteNonQuery();
            con.Close();
        }
